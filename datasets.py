...
class DRIVER_TEST(Dataset):
    # AFLW dataset with flipping
    def __init__(self, data_dir, filename_path, transform, direction, flip=False, crop=False, image_mode='RGB'):
        self.data_dir = data_dir
        self.transform = transform
        self.image_mode = image_mode
    ...
    def __getitem__(self, index):
        img_name = os.path.join(self.data_dir, self.X_train[index].split(' ')[0])

        if os.path.splitext(img_name)[1] == '.jpg':
            if self.crop:
                x_min = (int)(self.y_train[index].split(' ')[1])
                y_min = (int)(self.y_train[index].split(' ')[2])
                x_max = x_min + (int)(self.y_train[index].split(' ')[3])
                y_max = x_max + (int)(self.y_train[index].split(' ')[4])

                yaw = (float)(self.y_train[index].split(' ')[5])
                pitch = (float)(self.y_train[index].split(' ')[6])
                roll = (float)(self.y_train[index].split(' ')[7])
                if self.direction:
                    direction_value = (int)(self.y_train[index].split(' ')[8])
            else:
                yaw = (float)(self.y_train[index].split(' ')[1])
                pitch = (float)(self.y_train[index].split(' ')[2])
                roll = (float)(self.y_train[index].split(' ')[3])
                if self.direction:
                    direction_value = (int)(self.y_train[index].split(' ')[4])

        else:
            img_name = img_name + ' ' + self.X_train[index].split(' ')[1]

            if self.crop:
                x_min = (int)(self.y_train[index].split(' ')[2])
                y_min = (int)(self.y_train[index].split(' ')[3])
                x_max = x_min + (int)(self.y_train[index].split(' ')[4])
                y_max = x_max + (int)(self.y_train[index].split(' ')[5])

                yaw = (float)(self.y_train[index].split(' ')[6])
                pitch = (float)(self.y_train[index].split(' ')[7])
                roll = (float)(self.y_train[index].split(' ')[8])
                if self.direction:
                    direction_value = (int)(self.y_train[index].split(' ')[9])
            else:
                yaw = (float)(self.y_train[index].split(' ')[2])
                pitch = (float)(self.y_train[index].split(' ')[3])
                roll = (float)(self.y_train[index].split(' ')[4])
                if self.direction:
                    direction_value = (int)(self.y_train[index].split(' ')[5])
        ...
        labels = torch.LongTensor(np.digitize([yaw, pitch, roll], bins) - 1)
        if self.direction:
            cont_labels = torch.FloatTensor([yaw, pitch, roll, direction_value])
        else:
            cont_labels = torch.FloatTensor([yaw, pitch, roll])
        ...
        # print 'progress cur/sum : %d/%d' % (index, self.length)

        return img, labels, cont_labels, img_name
...
class Driver_Train_TXT(Dataset):
    def __init__(self, filename_path, transform, image_mode='RGB'):
        self.transform = transform
        filename_list = get_list_from_filenames(filename_path)
    ...
    def __getitem__(self, index):
        ...
        if len(img_name.split(' ')) > 1:
            if os.path.splitext(img_name.split(' ')[1])[1] == '.jpg':
        ...
        direction_label = self.X_train[index].split(' ')[-1]
        ...
        return img, img_name, direction_label, self.X_train[index]
